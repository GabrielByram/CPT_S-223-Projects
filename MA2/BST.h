...
protected:
	...
	void printLevelOrderHelper(Node<T>* root, std::ostream& out) {
		std::queue<Node<int>*> q1;
		Node<int>* current;
		q1.push(root);
		
		while(!q1.empty()) {
			current = q1.front(); // get front value
			q1.pop(); // release value now that we have it
			out << current->value << " ";
...
public:
	...
	BST(BST&& other) : _root(nullptr) {
		...
		// remove other tree's pointer to root
		other->_root = nullptr;
	...
	BST& operator=(BST& other) {
		// make sure it isn't assigned to itself
		if (&other == this)
			return *this;
 
		// delete current data of tree
		delete _root;
